...
尚未完成所有模块、复杂闭环及多层子系统的完整数值回归。新版与旧版模型暂不互相导入。

2026-09-12 的界面清理去掉首页试用标记、宿主编辑器说明栏和旧版跳转按钮，并删除模型列表中的迁移说明。Agent 面板与画布帮助按钮恢复为上游实现。此次清理保留既有平台登录、模型保存和仿真适配，不表示代码与上游完全相同；发布及同步清单位于 M2PLab 工作区 `deploy/releases/m2plink-clean-20260912/`。
